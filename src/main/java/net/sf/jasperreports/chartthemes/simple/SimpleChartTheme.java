...
import net.sf.jasperreports.chartthemes.provider.IApplyProperties;
...
/*
 * @version $Id: SimpleChartTheme.java 7197 2014-08-27 11:59:50Z teodord $
 */
public class SimpleChartTheme implements ChartTheme {
	...
	protected void handlePlotAxisLocations(CategoryPlot plot) {
		CategoryPlot categoryPlot = ((CategoryPlot) plot);
		AxisSettings settings = getRangeAxisSettings();
		if (settings != null) {
			AxisLocation location = settings.getLocation();
			if (location != null) {
				categoryPlot.setRangeAxisLocation(location);
			}
		}
		settings = getDomainAxisSettings();
		if (settings != null) {
			AxisLocation location = settings.getLocation();
			if (location != null) {
				categoryPlot.setDomainAxisLocation(location);
			}
		}
	}
	...
	protected void configurePlot(Plot plot, JRChartPlot jrPlot) {
		...
		if (plot instanceof CategoryPlot) {
			...
			handlePlotAxisLocations((CategoryPlot) plot);
			setRangeMarker((CategoryPlot) plot);
			configureRenderer(plot);
		}

		if (plot instanceof XYPlot) {
			handleXYPlotSettings((XYPlot) plot, jrPlot);
			handleSkipNullOrientation((XYPlot) plot);
		}
	}

	private void configureRenderer(Plot plot) {
		if (plot instanceof CategoryPlot) {
			IRendererProvider rendererProvider = getPlotSettings().getRendererProvider();
			if (rendererProvider instanceof IApplyProperties) {
				((IApplyProperties) rendererProvider).applyProperties();
			}
			if (rendererProvider instanceof IRendererProvider) {
				((CategoryPlot) plot).setRenderer(((IRendererProvider) rendererProvider).getRenderer());
			}

			List<Double> list = getPlotSettings().getSeriesBarWidthSequence();
			if (list != null) {
				// Set layered bar renderer
				LayeredBarRenderer renderer = new LayeredBarRenderer();
				renderer.setDrawBarOutline(false);

				StandardGradientPaintTransformer transformer = getPlotSettings().getGradientPaintTransformer();
			...
			handleBaseItemForPlot(((CategoryPlot) plot).getRenderer());
		}
	}

	private void handleBaseItemForPlot(CategoryItemRenderer renderer) {
		PlotSettings plotSettings = getPlotSettings();
		if (Boolean.TRUE.equals(plotSettings.getBaseItemLabelsVisible())) {
			renderer.setBaseItemLabelFont(getFontUtil().getAwtFont(plotSettings.getBaseItemLabelFont(), getLocale()));
			renderer.setBaseItemLabelPaint(plotSettings.getBaseItemLabelPaint().getPaint());
			renderer.setBaseItemLabelsVisible(true);
			renderer.setBasePositiveItemLabelPosition(plotSettings.getItemLabelPosition());
		}
	}

	protected void handleSkipNullOrientation(XYPlot plot) {
		if (plot != null) {
			processAxisWithBool(plot.getRangeAxis(), getRangeAxisSettings().getAutoAdjustToZero());
			processAxisWithBool(plot.getDomainAxis(), getDomainAxisSettings().getAutoAdjustToZero());
		}
	}

	private void processAxisWithBool(ValueAxis axis, Boolean bool) {
		if (bool != null && axis instanceof NumberAxis) {
			((NumberAxis) axis).setAutoRangeIncludesZero(bool);
		}
	}

	protected void setRangeMarker(CategoryPlot plot) {
		Marker marker = getPlotSettings().getMarker();
		if (marker != null) {
	...
	/**
	 * Sets all the axis formatting options. This includes the colors and fonts
	 * to use on the axis as well as the color to use when drawing the axis
	 * line.
	 *
	 * @param axis
	 *            the axis to format
	 ...
	 * @param tickLabelFont
	 *            the font to use for each tick mark value label
	 * @param tickLabelColor
	 *            the color of each tick mark value label
	 * @param tickLabelMask
	 *            formatting mask for the label. If the axis is a NumberAxis
	 *            then the mask should be <code>java.text.DecimalFormat</code>
	 *            mask, and if it is a DateAxis then the mask should be a
	 *            <code>java.text.SimpleDateFormat</code> mask.
	 * @param verticalTickLabels
	 *            flag to draw tick labels at 90 degrees
	 * @param lineColor
	 *            color to use when drawing the axis line and any tick marks
	 * @param isRangeAxis
	 *            used to distinguish between range and domain axis type
	 */
	protected void configureAxis(Axis axis, JRFont labelFont, Color labelColor, JRFont tickLabelFont,
		...
		configureAxis(axis, labelFont, labelColor, tickLabelFont, tickLabelColor, tickLabelMask, verticalTickLabels,
				lineColor, axisSettings, DateTickUnit.YEAR, axisMinValue, axisMaxValue);
	}
	 ...
	 *            used to distinguish between range and domain axis type
	 * @param timeUnit
	 *            time unit used to create a DateAxis
	 */
	protected void configureAxis(Axis axis, JRFont labelFont, Color labelColor, JRFont tickLabelFont,
			Color tickLabelColor, String tickLabelMask, Boolean verticalTickLabels, Paint lineColor,
	...
	protected JFreeChart createAreaChart() throws JRException {
		...
		JFreeChart jfreeChart = ChartFactory.createAreaChart(evaluateTextExpression(getChart().getTitleExpression()),
				...
				(CategoryDataset) getDataset(), getPlot().getOrientationValue().getOrientation(), isShowLegend(), true,
				false);
		...
		configureAxis(((CategoryPlot) jfreeChart.getPlot()).getDomainAxis(), areaPlot.getCategoryAxisLabelFont(),
				...
				getDomainAxisSettings(), (Comparable<?>) evaluateExpression(areaPlot.getDomainAxisMinValueExpression()),
		...
		configureAxis(((CategoryPlot) jfreeChart.getPlot()).getRangeAxis(), areaPlot.getValueAxisLabelFont(),
				...
				areaPlot.getValueAxisVerticalTickLabels(), areaPlot.getOwnValueAxisLineColor(), getRangeAxisSettings(),
	...
	protected JFreeChart createBar3DChart() throws JRException {
		ChartFactory.setChartTheme(StandardChartTheme.createLegacyTheme());
		JFreeChart jfreeChart = ChartFactory.createBarChart3D(evaluateTextExpression(getChart().getTitleExpression()),
	...
	protected JFreeChart createBarChart() throws JRException {
		...
		configureAxis(categoryPlot.getRangeAxis(), barPlot.getValueAxisLabelFont(), barPlot.getValueAxisLabelColor(),
		...
		if (isShowLabels) {
			if (rangeAxisMaxValue == null) {
				// in case the bars are horizontal and there was no range max
				// value specified,
				// we try to make the axis a little longer for labels to fit on
				// the plot
						...
						if (barPlot.getOrientationValue() == PlotOrientationEnum.HORIZONTAL) {
							rangeAxisMaxRatio = JRPropertiesUtil
									...
									.getFloatProperty(getChart(),
											"net.sf.jasperreports.chart.bar.horizontal.range.max.value.ratio", 1.25f);
						} else {
							rangeAxisMaxRatio = JRPropertiesUtil
									.getInstance(getChartContext().getJasperReportsContext())
									.getFloatProperty(getChart(),
	...
	protected JFreeChart createBubbleChart() throws JRException {
		ChartFactory.setChartTheme(StandardChartTheme.createLegacyTheme());
		JFreeChart jfreeChart = ChartFactory.createBubbleChart(evaluateTextExpression(getChart().getTitleExpression()),
				evaluateTextExpression(((JRBubblePlot) getPlot()).getXAxisLabelExpression()),
				evaluateTextExpression(((JRBubblePlot) getPlot()).getYAxisLabelExpression()), (XYZDataset) getDataset(),
	...
	protected JFreeChart createHighLowChart() throws JRException {
		ChartFactory.setChartTheme(StandardChartTheme.createLegacyTheme());
		JFreeChart jfreeChart = ChartFactory.createHighLowChart(evaluateTextExpression(getChart().getTitleExpression()),
		...
		// Handle the axis formating for the category axis
		configureAxis(xyPlot.getDomainAxis(), highLowPlot.getTimeAxisLabelFont(), highLowPlot.getTimeAxisLabelColor(),
				highLowPlot.getTimeAxisTickLabelFont(), highLowPlot.getTimeAxisTickLabelColor(),
				highLowPlot.getTimeAxisTickLabelMask(), highLowPlot.getTimeAxisVerticalTickLabels(),
		...
		// Handle the axis formating for the value axis
		configureAxis(xyPlot.getRangeAxis(), highLowPlot.getValueAxisLabelFont(), highLowPlot.getValueAxisLabelColor(),
				highLowPlot.getValueAxisTickLabelFont(), highLowPlot.getValueAxisTickLabelColor(),
				highLowPlot.getValueAxisTickLabelMask(), highLowPlot.getValueAxisVerticalTickLabels(),
	...
	protected JFreeChart createLineChart() throws JRException {
		...
		if (categoryItemRenderer instanceof LineAndShapeRenderer) {
			LineAndShapeRenderer lineRenderer = (LineAndShapeRenderer) categoryItemRenderer;
			...
			lineRenderer.setBaseShapesVisible(isShowShapes);// FIXMECHART check
		...
		configureAxis(categoryPlot.getDomainAxis(), linePlot.getCategoryAxisLabelFont(),
				...
				getDomainAxisSettings(), (Comparable<?>) evaluateExpression(linePlot.getDomainAxisMinValueExpression()),
				(Comparable<?>) evaluateExpression(linePlot.getDomainAxisMaxValueExpression()));

		// Handle the axis formating for the value axis
		configureAxis(categoryPlot.getRangeAxis(), linePlot.getValueAxisLabelFont(), linePlot.getValueAxisLabelColor(),
				linePlot.getValueAxisTickLabelFont(), linePlot.getValueAxisTickLabelColor(),
				linePlot.getValueAxisTickLabelMask(), linePlot.getValueAxisVerticalTickLabels(),
	...
	protected JFreeChart createPie3DChart() throws JRException {
		ChartFactory.setChartTheme(StandardChartTheme.createLegacyTheme());
		JFreeChart jfreeChart = ChartFactory.createPieChart3D(evaluateTextExpression(getChart().getTitleExpression()),
		...
		double depthFactor = jrPie3DPlot.getDepthFactorDouble() == null ? JRPie3DPlot.DEPTH_FACTOR_DEFAULT
				: jrPie3DPlot.getDepthFactorDouble().doubleValue();
	...
	protected JFreeChart createScatterChart() throws JRException {
		ChartFactory.setChartTheme(StandardChartTheme.createLegacyTheme());
		JFreeChart jfreeChart = ChartFactory.createScatterPlot(evaluateTextExpression(getChart().getTitleExpression()),
				evaluateTextExpression(((JRScatterPlot) getPlot()).getXAxisLabelExpression()),
				evaluateTextExpression(((JRScatterPlot) getPlot()).getYAxisLabelExpression()), (XYDataset) getDataset(),
				getPlot().getOrientationValue().getOrientation(), isShowLegend(), true, false);
		...
		configureAxis(jfreeChart.getXYPlot().getDomainAxis(), scatterPlot.getXAxisLabelFont(),
				...
				scatterPlot.getXAxisVerticalTickLabels(), scatterPlot.getOwnXAxisLineColor(), getDomainAxisSettings(),
		...
		configureAxis(jfreeChart.getXYPlot().getRangeAxis(), scatterPlot.getYAxisLabelFont(),
				...
				scatterPlot.getYAxisVerticalTickLabels(), scatterPlot.getOwnYAxisLineColor(), getRangeAxisSettings(),
	...
	protected JFreeChart createXyAreaChart() throws JRException {
		ChartFactory.setChartTheme(StandardChartTheme.createLegacyTheme());
		JFreeChart jfreeChart = ChartFactory.createXYAreaChart(evaluateTextExpression(getChart().getTitleExpression()),
				evaluateTextExpression(((JRAreaPlot) getPlot()).getCategoryAxisLabelExpression()),
				evaluateTextExpression(((JRAreaPlot) getPlot()).getValueAxisLabelExpression()),
	...
	protected JFreeChart createXYBarChart() throws JRException {
		...
		ChartFactory.setChartTheme(StandardChartTheme.createLegacyTheme());
		JFreeChart jfreeChart = ChartFactory.createXYBarChart(evaluateTextExpression(getChart().getTitleExpression()),
		...
		// Handle the axis formating for the category axis
		configureAxis(xyPlot.getDomainAxis(), barPlot.getCategoryAxisLabelFont(), barPlot.getCategoryAxisLabelColor(),
				barPlot.getCategoryAxisTickLabelFont(), barPlot.getCategoryAxisTickLabelColor(),
				barPlot.getCategoryAxisTickLabelMask(), barPlot.getCategoryAxisVerticalTickLabels(),
	...
	protected JFreeChart createXyLineChart() throws JRException {
		...
		ChartFactory.setChartTheme(StandardChartTheme.createLegacyTheme());
		JFreeChart jfreeChart = ChartFactory.createXYLineChart(evaluateTextExpression(getChart().getTitleExpression()),
		...
		configureAxis(jfreeChart.getXYPlot().getRangeAxis(), linePlot.getValueAxisLabelFont(),
				...
				linePlot.getValueAxisVerticalTickLabels(), linePlot.getOwnValueAxisLineColor(), getRangeAxisSettings(),
				(Comparable<?>) evaluateExpression(linePlot.getRangeAxisMinValueExpression()),
				(Comparable<?>) evaluateExpression(linePlot.getRangeAxisMaxValueExpression()));
	...
	protected JFreeChart createGanttChart() throws JRException {
		...
		ChartFactory.setChartTheme(StandardChartTheme.createLegacyTheme());
		JFreeChart jfreeChart = ChartFactory.createGanttChart(evaluateTextExpression(getChart().getTitleExpression()),
				evaluateTextExpression(((JRBarPlot) getPlot()).getCategoryAxisLabelExpression()),
				evaluateTextExpression(((JRBarPlot) getPlot()).getValueAxisLabelExpression()),
		...
		// FIXMETHEME these are useless if the theme settings apply after
		// regardless of these; check all
		categoryPlot.getDomainAxis().setTickMarksVisible(isShowTickMarks);
		categoryPlot.getDomainAxis().setTickLabelsVisible(isShowTickLabels);
		// Handle the axis formating for the category axis
		configureAxis(categoryPlot.getDomainAxis(), barPlot.getCategoryAxisLabelFont(),
	...
	/**
	 * Converts a JasperReports meter interval to one that JFreeChart
	 * understands.
	 *
	 * @param interval
	...
	protected JFreeChart createMeterChart() throws JRException {
		...
		// Set the spacing between ticks. I hate the name "tickSize" since to me
		// it
		// implies I am changing the size of the tick, not the spacing between
		// them.
		...
		JFreeChart jfreeChart = new JFreeChart(evaluateTextExpression(getChart().getTitleExpression()), null, chartPlot,
				isShowLegend());
	...
	/**
	 * Build and run a thermometer chart. JFreeChart thermometer charts have
	 * some limitations. They always have a maximum of three ranges, and the
	 * colors of those ranges seems to be fixed.
	 *
	 * @param evaluation
	 *            current expression evaluation phase
	 * @throws JRException
	...
	protected JFreeChart createDialChart() throws JRException {
		...
		Range range = convertRange(jrPlot.getDataRange());
		// double bound = Math.max(Math.abs(range.getUpperBound()),
		...
		JFreeChart jfreeChart = new JFreeChart(evaluateTextExpression(getChart().getTitleExpression()), null, dialPlot,
				isShowLegend());
	...
	/**
	 * Specifies the axis location. It has to be overridden for child themes
	 * with another default axis location
	 *
	 * protected AxisLocation getChartAxisLocation(JRChartAxis
	 * chartAxis)//FIXMETHEME { if(chartAxis.getPositionByte() != null) { switch
	 * (chartAxis.getPositionByte().byteValue()) { case
	 * JRChartAxis.POSITION_RIGHT_OR_BOTTOM : return
	 * AxisLocation.BOTTOM_OR_RIGHT; default: return AxisLocation.TOP_OR_LEFT; }
	 * } else { return (AxisLocation)getDefaultValue(defaultAxisPropertiesMap,
	 * ChartThemesConstants.AXIS_LOCATION); } }
	 * 
	 * /**
	 *
	 */
...
//					DefaultDrawingSupplier.DEFAULT_PAINT_SEQUENCE.length);
	...
	protected void setChartBackground(JFreeChart jfreeChart) {
		...
		// GradientPaint gp = backgroundPaint instanceof GradientPaint ?
		// (GradientPaint) backgroundPaint
		// : null;
		// if (gp != null) {
		// backgroundPaint = gp;/*new GradientPaint(0f, 0f, gp.getColor1(), 0f,
		// getChart().getHeight() * 0.7f, gp.getColor2(), false);*/
		// }
	...
	protected void setChartSubtitles(JFreeChart jfreeChart) throws JRException {
		...
		if (subtitleVisibility == null || subtitleVisibility.booleanValue()) {
			...
			if (subtitleText != null) {
				...
				Paint subtitleForecolor = getChart().getOwnSubtitleColor() != null ? getChart().getOwnSubtitleColor()
						: subtitleSettings.getForegroundPaint() != null
								? subtitleSettings.getForegroundPaint().getPaint() : getChart().getSubtitleColor();
				// Subtitle has not its own position set, and by default this
				// will be set the same as title position
	...
	protected void setChartLegend(JFreeChart jfreeChart) {
		// The legend visibility is already taken into account in the jfreeChart
		...
		if (legend != null) {
			...
			Float border = legendSettings.getBorder();
			if (border != null) {
				legend.setBorder(border, border, border, border);
			}
	...
	protected void setChartBorder(JFreeChart jfreeChart) {
		...
		if (lineBox.getLeftPen().getLineWidth().floatValue() == 0
			...
			if (isVisible) {
				...
				Paint paint = chartSettings.getBorderPaint() == null ? null : chartSettings.getBorderPaint().getPaint();
				if (paint != null)
	...
	protected void handleCategoryPlotSettings(CategoryPlot p, JRChartPlot jrPlot) {
		...
		if (hasRotation) {
			double labelRotation = jrPlot.getLabelRotationDouble() != null
					? jrPlot.getLabelRotationDouble().doubleValue() : themeLabelRotation.doubleValue();
		...
		PlotOrientation plotOrientation = jrPlot.getOrientationValue() != null
				? jrPlot.getOrientationValue().getOrientation() : plotSettings.getOrientation();
		if (plotOrientation != null) {
			p.setOrientation(plotOrientation);
		}
		CategoryItemRenderer categoryRenderer = p.getRenderer();
		if (categoryRenderer instanceof StackedBarRenderer) {
			StandardGradientPaintTransformer type = plotSettings.getGradientPaintTransformer();
			if (type != null) {
				((BarRenderer) categoryRenderer).setGradientPaintTransformer(type);
			}
		}

		if (categoryRenderer instanceof AbstractRenderer) {
	...
	protected void setAxisTickLabels(Axis axis, JRFont tickLabelFont, Paint tickLabelColor, String tickLabelMask,
		...
		boolean axisTickLabelsVisible = axisSettings.getTickLabelsVisible() == null
				|| axisSettings.getTickLabelsVisible().booleanValue();
	...
	protected void setAxisTickMarks(Axis axis, Paint lineColor, AxisSettings axisSettings) {
		...
		if (axisTickMarksVisible) {
			...
			Paint tickMarkPaint = axisSettings.getTickMarksPaint() != null
					&& axisSettings.getTickMarksPaint().getPaint() != null ? axisSettings.getTickMarksPaint().getPaint()
							: lineColor;
	...
	protected void setAxisBounds(Axis axis, AxisSettings axisSettings, int timePeriodUnit, Comparable<?> minValue,
			Comparable<?> maxValue) throws JRException {
		if (axis instanceof ValueAxis) {
			if (axis instanceof DateAxis) {
				if (minValue != null) {
	...
	/**
	 * For a given axis, adjust the tick unit size, in order to have a
	 * customizable number of ticks on that axis
	 */
				...
				if (tickInterval != null) {
					if (numberAxis.getNumberFormatOverride() != null) {
						numberAxis.setTickUnit(
								new NumberTickUnit(tickInterval.doubleValue(), numberAxis.getNumberFormatOverride()));
					} else {
	...
	protected boolean isShowLegend() {
		Boolean showLegend = getChart().getShowLegend() == null
				? getLegendSettings().getShowLegend() == null ? Boolean.TRUE : getLegendSettings().getShowLegend()
	...
	protected Paint[] getPaintSequence(PlotSettings plotSettings, JRChartPlot jrPlot) {
		...
		List<PaintProvider> themeSeriesPaintProvider = getChartThemeSettings().getPlotSettings()
				.getSeriesGradientPaintSequence() != null
	...
	protected Stroke[] getOutlineStrokeSequence(PlotSettings plotSettings) {
		List<Stroke> outlineStrokeSequenceList = getChartThemeSettings().getPlotSettings()
